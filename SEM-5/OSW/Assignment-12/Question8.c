...
void process_code(int process_number, const char *message, int semaphore_index) {
...
int main() {
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        if (pid == -1) {
            ...
            process_code(i, (i == 1) ? "Coronavirus" : (i == 2) ? "WHO:" : (i == 3) ? "COVID-19" : (i == 4) ? "disease" : "pandemic", i - 1);
            exit(EXIT_SUCCESS);
        }
    }
